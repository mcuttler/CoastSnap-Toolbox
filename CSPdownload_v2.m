...
function [] = CSPdownload_v2(site, siteDB, base_path, image_path); 
...
data = importdata(fullfile(db_path,[site '_download.csv'])); 
...
for i = 2:size(data,1)
    try
        ...
        author_name = [author_name; strip(cdum(2),'"')];         
...
%             
%             %set outfile name
%             if ~isnat(data.CoastSnaps_Post_Date(i))
...
% 
%             %determine timestamp 
%             if ~isempty(data.Author{i}) %if Artist has data then timestamp is in Author
%                 dtime = [data.Author{i}(1:4) data.Author{i}(6:7) data.Author{i}(9:10) '_' data.Author{i}(12:13) data.Author{i}(15:16) data.Author{i}(18:19)]; 
%                 if strcmp(data.AmazonS3{i}(end-4:end),'.jpeg')
%                     outfile = [siteDB '_' dtime data.AmazonS3{i}(end-4:end)]; 
%                 else
%                     outfile = [siteDB '_' dtime data.AmazonS3{i}(end-3:end)]; 
%                 end
%             else
%                 if strcmp(data.AmazonS3{i}(end-4:end),'.jpeg')
...
%             
%             %save filename and author for CoastSnapDB
%             if isempty(data.Artist{i})
%                 author_name = [author_name; data.Author(i)]; 
%             else
%                 author_name = [author_name; data.Artist{i}]; 
%             end            
%             im_filename = [im_filename; {outfile}]; 
%         catch
%             author_name = [author_name; 'could not read file'];
